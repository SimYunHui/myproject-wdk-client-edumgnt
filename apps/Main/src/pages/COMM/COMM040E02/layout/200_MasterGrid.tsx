...
export const getGridValues = (): RowObject => {
  ...
  // 유효성 검사 설정
  gridView.onValidateColumn = (_grid, column, _inserting, value) => {
...
const MasterGrid = forwardRef<MasterGridHandler, MasterGridProps>(
  ({ originRows, onAddRow, onSelectRow }, ref) => {
    ...
    useEffect(() => {
      ...
      gridView.onCurrentRowChanged = function (grid, oldRow, newRow) {
        if (newRow !== oldRow && newRow !== -1) {
          ...
          onSelectRow(
            curr.dataRow,
            grid.getDataSource().getJsonRow(curr.dataRow, true)
          );
        }
      };

      masterGrid.setBoolColumn(FIELD.REP_BUSI_PLACE_YN);
      // 그리드 정렬
      masterGrid.orderBy([FIELD.BUSI_PLACE], ["ascending"]);
      ...
      masterGrid.onEditCommit((_grid, _index, _oldValue, _newValue) => {
